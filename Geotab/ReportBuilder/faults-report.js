...
function loadDevices(selectedGroups = [], endpoint = '/api/devices') {
    ...
    // Update the diagnostics loading section
    fetch('/api/diagnostics')
        ...
        .then(data => {
                ...
                <div class="mb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="input-group w-auto">
                            ...
                            <input type="text" class="form-control" id="diagnosticSearch" 
                ...
                <div class="diagnostic-items">
                    ${diagnostics.map(diagnostic => `
                        <div class="diagnostic-item">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" value="${diagnostic.id}" 
                                    id="diagnostic_${diagnostic.id}" name="diagnostics">
            ...
            const searchInput = document.getElementById('diagnosticSearch');
            searchInput.addEventListener('input', (e) => {
                const searchTerm = e.target.value.toLowerCase();
                document.querySelectorAll('.diagnostic-item').forEach(item => {
                    const diagnosticText = item.textContent.toLowerCase();
                    item.style.display = diagnosticText.includes(searchTerm) ? '' : 'none';
                });
            });

            // Add diagnostic selection handlers
            document.getElementById('selectAllRules').onclick = () => {
                const visibleRules = Array.from(document.querySelectorAll('.diagnostic-item'))
...
fetch('/api/available-faults-columns')
    .then(response => response.json())
    .then(data => {
    ...
    reportForm.onsubmit = async (e) => {
        ...
        if (selectedDiagnostics.length === 0) {
            showStatus('Please select at least one diagnostic type', true);
            return;
        }
        ...
        try {
            const response = await fetch('/api/generate-faults-report', {
                method: 'POST',
